...
# Load config JSON
input_file = os.path.join("Input-Layouts", "room-layout-1.json")
...
def save_layouts(layouts: List[List[Dict]], prefix: str = "relationship_optimized_layout"):
    ...
    for i, layout in enumerate(layouts, 1):
        ...
        path = "Outputs/GA/Multiple-Layouts"
        os.makedirs(path, exist_ok=True)
        output_path = os.path.join(path, filename)
        with open(output_path, "w") as f:
            json.dump(output_data, f, indent=2)

        print(f"Multiple-Layout-Relationship-optimized layout {i} saved to {output_path}")
...
if __name__ == "__main__":
    ...
    os.path.exists("Outputs/GA/Multiple-Layouts") or os.makedirs("Outputs/GA/Multiple-Layouts")

    optimized_layouts_path = "Outputs/GA/Multiple-Layouts/Multiple-Layout-Relationship-optimized-layouts.json"
